...
# Conditional UI import: only load UI modules when not in headless streaming mode.
# Headless live streaming mode is activated by --input-source.
_ui_loaded = False
ui = None

if '--input-source' not in sys.argv:
    try:
        import modules.ui as ui
        _ui_loaded = True
    except Exception as e:
        print(f'[DLC.CORE] Warning: Could not load UI module: {e}')
        ui = None
...
def parse_args() -> None:
    ...
    program.add_argument('--mouth-mask', help='mask the mouth region (preserve original lips)', dest='mouth_mask', action='store_true', default=False)
    program.add_argument('--eyes-mask', help='mask the eyes region (preserve original eye movement/blinks)', dest='eyes_mask', action='store_true', default=False)
    program.add_argument('--eyebrows-mask', help='mask the eyebrows region (preserve original eyebrow expressions)', dest='eyebrows_mask', action='store_true', default=False)
    program.add_argument('--mouth-mask-size', help='mouth mask size 0-100 (0=off, 100=mouth to chin)', dest='mouth_mask_size', type=float, default=0.0)
    program.add_argument('--eyes-mask-size', help='eyes mask size 0-100 (0=off, 100=full eyes)', dest='eyes_mask_size', type=float, default=0.0)
    program.add_argument('--eyebrows-mask-size', help='eyebrows mask size 0-100 (0=off, 100=full brows)', dest='eyebrows_mask_size', type=float, default=0.0)
    ...
    # === START: Headless Streaming Arguments ===
    program.add_argument('--input-source', help='Input source for headless live mode: camera index (0), video file path, pipe, or stream URL (rtmp://...)', dest='input_source', default=None)
    program.add_argument('--stream-output', help='Output stream URL: rtmp://server/live/stream, udp://host:port, or file path', dest='stream_output', default=None)
    program.add_argument('--stream-width', help='Output stream width', dest='stream_width', type=int, default=1280)
    program.add_argument('--stream-height', help='Output stream height', dest='stream_height', type=int, default=720)
    ...
    modules.globals.headless = args.source_path or args.target_path or args.output_path or args.force_headless or args.input_source
    ...
    modules.globals.mouth_mask_size = args.mouth_mask_size
    modules.globals.eyes_mask_size = args.eyes_mask_size
    modules.globals.eyebrows_mask_size = args.eyebrows_mask_size
    ...
    modules.globals.input_source = args.input_source
    modules.globals.stream_output = args.stream_output
    modules.globals.stream_width = args.stream_width
    modules.globals.stream_height = args.stream_height
...
def run() -> None:
    ...
    # If --input-source is provided, run the headless live pipeline instead of GUI or batch processing.
    if modules.globals.input_source:
        print('[DLC.CORE] Headless live streaming mode activated')
        print(f'  Input: {modules.globals.input_source}')
        if modules.globals.stream_output:
            print(f'  Output: {modules.globals.stream_output}')
        print(f'  Resolution: {modules.globals.stream_width}x{modules.globals.stream_height}@{modules.globals.stream_fps}fps')
        from modules.headless_live import run_headless_live
        run_headless_live()
        return
    # === END: Headless Live Streaming Mode ===

    if modules.globals.headless:
        start()
    else:
        if ui is None:
            print('[DLC.CORE] ERROR: UI module not available. Use --headless or --input-source for headless mode.')
            return
